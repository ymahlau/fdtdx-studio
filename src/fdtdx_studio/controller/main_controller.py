...
class Controller:
    ...
    def saveGaussianPlaneSourceParams(self, parameters):
        ...
        if "wave_character" in parameters:
            parameters["wave_character"] = fdtdx.WaveCharacter(**parameters["wave_character"])
        if "switch" in parameters:
            parameters["switch"] = fdtdx.OnOffSwitch(**parameters["switch"])
    ...
    def add_material(self, Permittivity, Permeability, Electrical_conductivity, Magnetic_conductivity, name):
        ...
        self.model.material.create_new_material(
            permittivity=Permittivity,
            permability=Permeability,
            e_conductivity=Electrical_conductivity,
            m_conductivity=Magnetic_conductivity,
            name=name,
        )
    ...
    def ui_parse_objectlist_scrollarea(self):
        ...
        con = {
            ...
            "backend": s.backend,
            "dtype": s.dtype,
            "courant_factor": s.courant_factor,
            "gradient_config": s.gradient_config,
        }
    ...
    def save_constraints(self, object_name, cons):
        ...
        for key in delete:
            self.model.delete_constraint(key)
        for con in cons:
            match con["type"]:
                case "PositionConstraint":
                    self.model.add_pos_con(
                        object_name,
                        con["other_object"],
                        con["axes"],
                        con["object_positions"],
                        con["other_object_positions"],
                        con["margins"],
                        con["grid_margins"],
                        con["key"],
                    )
                case "SizeConstraint":
                    self.model.add_size_con(
                        object_name,
                        con["other_object"],
                        con["axes"],
                        con["other_axes"],
    ...
    def add_new_detector(self, *, detector_type: str, popup, typ: str, **kwargs):
        ...
        if "name" in filtered_kwargs:
            filtered_kwargs["name"] = self.model.namecheck(
                filtered_kwargs["name"]
            )  # checks if the name is unique and changes it if not
