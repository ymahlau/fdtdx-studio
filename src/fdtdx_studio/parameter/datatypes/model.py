...
import fdtdx
from fdtdx import ObjectContainer, SimulationConfig, SimulationObject, SimulationVolume, UniformMaterialObject
...
class Model(Constraints):
    ...
    def create_simulation_volume(self, xyz=(1e-5, 1e-5, 1e-5), material=fdtdx.Material()):
        self.track_object_list[0] = SimulationVolume(
            partial_real_shape=xyz, material=material, name="Simulation Volume"
        )
    ...
    def get_current_parameters(self):
        ...
        names = [
            o.name
            for o in self.track_object_list
            if not isinstance(o, fdtdx.PerfectlyMatchedLayer) and o.name != self.track_object_list[self.current].name
        ]
        return {
            **{f.name: f.value for f in self.track_object_list[self.current].get_public_fields()},
            "names": names,
    ...
    def create_mode_plane_source_obj(
        ...
        *,
        ...
        filter_pol=None,
        max_angle_random_offset=None,
        max_horizontal_offset=None,
        max_random_grid_offsets=None,
        max_random_real_offsets=None,
        max_vertical_offset=None,
        mode_index=None,
        name=None,
        partial_grid_shape=None,
        partial_real_shape=None,
    ...
    def real_position(self, obj: SimulationObject):
        ...
        self.constraints[key] = obj.place_relative_to(
            other=sim_vol,
            axes=axes,
            own_positions=tuple(0 for a in axes),
            other_positions=tuple(v for v in (x, y, z) if v is not None),
        )
