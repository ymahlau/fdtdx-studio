...
from fdtdx_studio.json_handling.load import Import
...
class Project:
    ...
    def save_Project_as(self):
        ...
        with ui.dialog() as dialogSaveWhere, ui.card():
            ...
            path = ui.input(
                ...
                on_change=lambda e: (self.set_name(e.value), update_button()),
            )
    ...
    def _remove_null_types(self, obj):
        ...
        if isinstance(obj, dict):
            ...
            return {k: self._remove_null_types(v) for k, v in obj.items() if type(v).__name__ != "Null"}
        elif isinstance(obj, list):
            # Rebuild list: keep items that are NOT Null, and recurse into items
            return [self._remove_null_types(v) for v in obj if type(v).__name__ != "Null"]
