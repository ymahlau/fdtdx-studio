...
class AutoConfigPanel(ObjectConfigPanel):
    ...
    def _create_element(self, definition: AttributeDef):
        ...
        if definition.ui_type == "number":
            ...
            element = SelectElement(
                definition.label, current_name, on_mat_change, options=options, tooltip=definition.tooltip
            )
        elif definition.ui_type == "nested":
            element = NestedObjectElement(
                definition.label,
                current_val,
                on_change_cb,
                on_navigate=lambda: self._navigate_to(key, definition.target_cls),
                tooltip=definition.tooltip,
            )
