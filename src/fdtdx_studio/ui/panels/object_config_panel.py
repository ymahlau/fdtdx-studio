...
class ObjectConfigPanel:
    ...
    def render_into(self, panel):
        ...
        with panel:
            with ui.column().classes("w-full gap-2 p-2"):
                ...
                ui.label("Partial Real Position").style("font-size: 13px; font-weight: bold;").tooltip(
                    "Sets center of box in relation to the simulation volume in real coordinates."
                )
                with ui.row().classes("w-full flex-nowrap gap-1"):
                    self.partial_real_position_x = (
                        ui.number("x", value=0, step=1e-6, validation=self._validate_float)
                        .classes("flex-1")
                        .props("dense")
                    )
                    self.partial_real_position_y = (
                        ui.number("y", value=0, step=1e-6, validation=self._validate_float)
                        .classes("flex-1")
                        .props("dense")
                    )
                    self.partial_real_position_z = (
                        ui.number("z", value=0, step=1e-6, validation=self._validate_float)
                        .classes("flex-1")
                        .props("dense")
                    )

                ui.label("Partial Real Shape").style("font-size: 13px; font-weight: bold;").tooltip(
                    "Sets the size of the box in real coordinates."
                )
                with ui.row().classes("w-full flex-nowrap gap-1"):
                    self.partial_real_shape_x = (
                        ui.number("x", value=1, min=0, step=1e-6, validation=self._validate_shape)
                        .classes("flex-1")
                        .props("dense")
                    )
                    self.partial_real_shape_y = (
                        ui.number("y", value=1, min=0, step=1e-6, validation=self._validate_shape)
                        .classes("flex-1")
                        .props("dense")
                    )
                    self.partial_real_shape_z = (
                        ui.number("z", value=1, min=0, step=1e-6, validation=self._validate_shape)
                        .classes("flex-1")
                        .props("dense")
                    )

                ui.label("Partial Grid Shape").style("font-size: 13px; font-weight: bold;").tooltip(
                    "Sets the size of the box in grid coordinates."
                )
                with ui.row().classes("w-full flex-nowrap gap-1"):
                    self.partial_grid_shape_x = (
                        ui.number("x", value=0, min=0, step=1e-6).classes("flex-1").props("dense")
                    )
                    self.partial_grid_shape_y = (
                        ui.number("y", value=0, step=1e-6, min=0).classes("flex-1").props("dense")
                    )
                    self.partial_grid_shape_z = (
                        ui.number("z", value=0, step=1e-6, min=0).classes("flex-1").props("dense")
                    )

                ui.label("Max Random Real Offsets").style("font-size: 13px; font-weight: bold;").tooltip(
                    "Maximum random offset in real coordinates."
                )
                with ui.row().classes("w-full flex-nowrap gap-1"):
                    self.max_random_real_offsets_x = (
                        ui.number("x", value=0, step=1e-6, validation=self._validate_float)
                ...
                ui.label("Max Random Grid Offsets").style("font-size: 13px; font-weight: bold;").tooltip(
                    "Maximum random offset in grid coordinates."
                )
                with ui.row().classes("w-full flex-nowrap gap-1"):
                    self.max_random_grid_offsets_x = (
                        ui.number("x", value=0, step=1e-6, validation=self._validate_float)
    ...
    def get_parameters(self):
        ...
        partial_real_position = _complete_triplet(
            self.partial_real_position_x, self.partial_real_position_y, self.partial_real_position_z
        )

        partial_real_shape = _complete_triplet(
            self.partial_real_shape_x, self.partial_real_shape_y, self.partial_real_shape_z
        )

        partial_grid_shape = _complete_triplet(
            self.partial_grid_shape_x, self.partial_grid_shape_y, self.partial_grid_shape_z
        )

        max_random_real_offsets = _complete_triplet(
            self.max_random_real_offsets_x, self.max_random_real_offsets_y, self.max_random_real_offsets_z
        )

        max_random_grid_offsets = _complete_triplet(
            self.max_random_grid_offsets_x, self.max_random_grid_offsets_y, self.max_random_grid_offsets_z
        )
    ...
    def add_con_to_scroll_and_dict(self, con):
        ...
        with self.scroll_area_con:
            with ui.row() as row:
                ui.button(
                    con["key"],
                    on_click=lambda: self.pop_up_con.open_pop_up(con["type"], self.name_of_all_objects, con),
                    color=None,
                ).props("flat")
