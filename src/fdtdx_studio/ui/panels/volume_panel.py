    ...
    def Volume_panel(self):
        ...
        with ui.dialog() as dialogVolume, ui.card():
            ...
            with ui.row().classes("items-center gap-1"):
                ...
                ui.icon("info_outline").classes("text-grey-5 cursor-help").style("font-size: 14px;").tooltip(
                    get_dyn_tooltip("partial_real_shape", "Sets the physical size of the simulation volume.")
                )

            with ui.row().style("padding-top: 0px").classes("justify-center"):
                x = (
                    ui.number("Width", value=(VTuple[0]), step=0.000001, validation=self._validate)
                    .classes("w-1/6")
                    .tooltip("Width of the simulation volume (m)")
                )
                y = (
                    ui.number("Height", value=(VTuple[1]), step=0.000001, validation=self._validate)
                    .classes("w-1/6")
                    .tooltip("Height of the simulation volume (m)")
                )
                z = (
                    ui.number("Length", value=(VTuple[2]), step=0.000001, validation=self._validate)
                    .classes("w-1/6")
                    .tooltip("Length of the simulation volume (m)")
                )
            ...
            with ui.row().classes("items-center gap-1"):
                ...
                ui.icon("info_outline").classes("text-grey-5 cursor-help").style("font-size: 14px;").tooltip(
                    "Quickly set the volume dimensions to common predefined sizes."
                )
            with ui.row():
                ui.button("Small", on_click=lambda: preset(5e-6, 5e-6, 5e-6)).tooltip("Sets size to 5µm x 5µm x 5µm")
                ui.button("Medium", on_click=lambda: preset(1e-5, 1e-5, 1e-5)).tooltip(
                    "Sets size to 10µm x 10µm x 10µm"
                )
                ui.button("Large", on_click=lambda: preset(1e-4, 1e-4, 1e-4)).tooltip(
                    "Sets size to 100µm x 100µm x 100µm"
                )

            with ui.row().classes("items-center gap-1"):
                ui.label("Material:").style("font-weight: bold;")
                ui.icon("info_outline").classes("text-grey-5 cursor-help").style("font-size: 14px;").tooltip(
                    get_dyn_tooltip("material", "Sets the background material of the simulation volume.")
                )
            with ui.dropdown_button(self.controller.model.material.get_name_from_material(self.material)).classes(
                "w-1/3"
            ) as self.material_show:
