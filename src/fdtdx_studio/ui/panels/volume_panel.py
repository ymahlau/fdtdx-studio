...
class volume_panel():
  def __init__(self, *args):
    if len(args) == 2:
      ...
      raise TypeError(f"volume_panel expects 1 or 2 arguments, got {len(args)}")
  ...
  def volume_param_panel(self, dialog: ui.dialog = None):
    ...
    with ui.row().style('padding-top: 0px').classes('w-full justify-center gap-1'):
      x = ui.number('Width', value=(VTuple[0]), step=0.000001, validation=self._validate).classes('flex-1').tooltip('Width of the simulation volume (m)')
      y = ui.number('Height', value=(VTuple[1]), step=0.000001, validation=self._validate).classes('flex-1').tooltip('Height of the simulation volume (m)')
      z = ui.number('Length', value=(VTuple[2]), step=0.000001, validation=self._validate).classes('flex-1').tooltip('Length of the simulation volume (m)')
    ...
    with ui.row().classes('w-full items-center gap-1'):
        ui.select(['Small', 'Medium', 'Large'], label='Preset Sizes', value=current_preset, on_change=lambda e: preset_changed(e.value)).classes('flex-1')
    ...
    with ui.row().classes('w-full items-center gap-1'):
        ui.select(mat_names, label='Material', value=current_mat, on_change=lambda e: self.choose_material_by_name(e.value)).classes('flex-1')
