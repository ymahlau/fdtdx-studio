...
def add_tooltip_icon(tooltip_text: str):
    ...
    if tooltip_text:
        ui.icon("info_outline").classes("text-grey-5 cursor-help").style("font-size: 14px; margin-left: 2px;").tooltip(
            tooltip_text
        )
...
class new_pop_up:
    ...
    def add_button(self, function, label="Save"):
        ...
        self.save = (
            ui.button("Save", on_click=function)
            .classes("w-full")
            .style("margin-top: 8px;")
            .on_click(lambda: self.controller.ui_update())
        )
