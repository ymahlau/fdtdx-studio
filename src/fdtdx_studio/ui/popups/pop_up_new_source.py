...
from fdtdx_studio.ui.popups.new_pop_up import _tip, add_tooltip_icon
...
class pop_up_new_source(NewPopUp):
    ...
    def _call_add(self):
        ...
        kwargs = dict(
            ...
            switch=dict(
                ...
                fixed_on_time_steps=(
                    None
                    if not self.switch_fixed_on_time_steps or not self.switch_fixed_on_time_steps.value
            ...
            if self.source_kind == "MODE" or self.fixed_E_x is None or self.fixed_E_y is None or self.fixed_E_z is None
            else [self.fixed_E_x.value, self.fixed_E_y.value, self.fixed_E_z.value],
            fixed_H_polarization_vector=None
            if self.source_kind == "MODE" or self.fixed_H_x is None or self.fixed_H_y is None or self.fixed_H_z is None
            else [self.fixed_H_x.value, self.fixed_H_y.value, self.fixed_H_z.value],
        )
            ...
            with ui.row():
                with ui.column():
                    ...
                    ui.label("Wave Character").style(
                        "font-size: 14px; font-weight: bold; margin-top: 8px; margin-bottom: 8px"
                    )
                ...
                with ui.column():
                    ...
                    ui.checkbox(
                        "Switch Is Always Off", on_change=lambda s: setattr(self, "switch_is_always_off", s.value)
                    )
    ...
    def make_source_mode_options(self):
        ...
        if self.source_kind == "MODE":
            ...
            ui.checkbox(
                "Normalize by Energy",
                value=self.normalize_by_energy,
                on_change=lambda s: setattr(self, "normalize_by_energy", s.value),
            )
            with ui.column():
                ui.label("Fixed E Polarization Vector").style(
                    "font-size: 14px; font-weight: bold; margin-top: 8px; margin-bottom: 8px"
                )
            ...
            with ui.column():
                ui.label("Fixed H Polarization Vector").style(
                    "font-size: 14px; font-weight: bold; margin-top: 8px; margin-bottom: 8px"
                )
