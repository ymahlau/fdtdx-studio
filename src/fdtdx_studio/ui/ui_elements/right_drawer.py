    ...
    def build(self):
        ...
        with ui.right_drawer(elevated=True).style("background-color: #E3E3E3") as self.right_drawer:
            with ui.column().classes("w-full h-full justify-between gap-1"):
                ...
                with (
                    ui.scroll_area()
                    .classes("justify-start items-start h-full")
                    .style("padding: 0px;") as self.config_panel
                ):
                ...
                ui.button("Simulation Parameters", on_click=self.show_sim_panel).classes("w-full").style(
                    "margin-bottom: 16px;"
                )
